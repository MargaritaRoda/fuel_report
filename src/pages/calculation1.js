...
const restFuel = 18; // получено из state
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (i === 0) {
      ...
      allFuel = arr[i].reduce((sum, item) => {
        return sum + item.fuel + (newRestFuel - 1);
      }, 0);
    }

    constantDistance = arr[i].reduce(
      (sum, item) => {
        if (item.distance !== 0) {
    ...
    if (resultDistance > 60) {
      ...
      newRestFuel =
        allFuel +
        1 -
        ((constantDistance.numDay * 60 + constantDistance.distance) * 7.2) /
          100;
    } else {
      newRestFuel = 1;
    }
    ...
    arr[i].map((item) => {
      if (item.distance === 0) {
        return (item.distance = resultDistance);
      }
    });
  }
